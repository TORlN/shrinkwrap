...
class TestConfigWiredIntoCLI:
    def test_default_level_from_config_applies_to_compress(
        self, tmp_path: Path
    ) -> None:
    ...
    def test_explicit_level_flag_overrides_config_default(
        self, tmp_path: Path
    ) -> None:
    ...
    def test_default_profile_from_config_applies_to_compress(
        self, tmp_path: Path
    ) -> None:
    ...
    def test_extra_immutable_keywords_from_config_applied(
        self, tmp_path: Path
    ) -> None:
    ...
    def test_missing_config_falls_back_to_normalize(
        self, tmp_path: Path
    ) -> None:
    ...
    def test_audit_uses_extra_immutable_keywords_from_config(
        self, tmp_path: Path
    ) -> None:
...
class TestExpandPreservesAnnotations:
    ...
    def test_expand_keeps_mutable_annotation_with_compression(
        self, tmp_path: Path
    ) -> None:
        src = tmp_path / "CLAUDE.md"
        src.write_text(
...
class TestExpandInPlace:
    def test_expand_in_place_restores_readable_markdown(
        self, tmp_path: Path
    ) -> None:
        src = tmp_path / "CLAUDE.md"
        src.write_text("## Status\n- ok\n")
    ...
    def test_expand_in_place_does_not_create_expanded_file(
        self, tmp_path: Path
    ) -> None:
        src = tmp_path / "CLAUDE.md"
        src.write_text("## Status\n- ok\n")
    ...
    def test_expand_in_place_and_output_are_mutually_exclusive(
        self, tmp_path: Path
    ) -> None:
        src = tmp_path / "CLAUDE.md"
        src.write_text("## Status\n- ok\n")
    ...
    def test_compress_then_expand_in_place_is_roundtrip(
        self, tmp_path: Path
    ) -> None:
