...
This section tracks what is actively being worked on right now. It gets updated frequently
as tasks are completed or priorities shift. Please keep this section current.
...
We discussed several approaches in the last planning session. After reviewing the options,
the team decided to move forward with the following. These decisions should be treated as
settled unless explicitly revisited in a planning meeting.
...
There are several things we haven't fully resolved yet. These are being actively discussed
and should not be treated as decided. If you need guidance on one of these, flag it.
...
The following items are prioritized and ready to pick up once current sprint work is done.
This list is maintained by the team lead and reflects the current agreed-upon order.
...
The local dev environment uses Docker Compose. Everything you need to run the stack
is in `docker-compose.yml` at the repo root. The following notes apply to all engineers.
